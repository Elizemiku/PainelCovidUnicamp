...
covid <- covid %>% mutate(MediaNovos = 0,
                          ObitosNovos = 0)
```
...
pickerInput(
  ...
  choices = c("casos", "casos_novos", "óbitos", "óbitos_novos", 
              "Novos Casos (Média Móvel)" = "MediaNovos", "Novos Óbitos (Média Móvel)" = "ObitosNovos"), 
  selected = "casos",
  multiple = FALSE,
...
filtro_cidades <- reactive({
    covid %>% filter(Cidade %in% input$Cidade) %>% mutate(MediaNovos = estatisticas_moveis(.$casos_novos),
                          ObitosNovos = estatisticas_moveis(.$óbitos_novos))
})
